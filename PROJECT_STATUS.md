...
Version: 6.41
Status: 入库质量诊断完成 + 前端题库页上线 + 脏数据清理 + 图片 URL 落库
Date: 2026-08-27
...
**v6.40-6.41（2026-08-27）新增**：
- **管理后台「题库管理」页面上线**：`/admin/questions` 目录树（学科→年级→
  题目数）+ 题目列表 + 点击展开详情 + 文档「入库」入口；后端新增
  `GET /api/admin/catalog`、`source_document_name` 筛选、序列化补学科/题型名称。
- **入库质量诊断**：详解缺失 548/921 approved（59%，源文档固有属性）、
  reviewing 86 题（82 题有答案，等 30 份 PDF 重跑自动修复）、图片历史 URL 已过期。
- **脏数据清理**：17 题（15 图片残留 + 2 地理选项混入题干）。
- **图片 URL 落库**：QuestionImage 新增 url 列（migration `20260827_0001`），
  对重跑后的新数据生效，前端有 URL 渲染实际图/无 URL 降级标识。
...
| 题库管理页（v6.40）+ 图片 URL 落库（v6.41） | ✅ 29 passed |
...
- **30 份样本口径**：原始 61% → 逐科调优 **79% (173/219)**。剩余缺口 10：
  ...
  物理 1（anchor）、历史 1（anchor）。**待 30 份 PDF 全量重跑刷新**（新代码
  表格拆行/膨胀放宽/答案区豁免已逐份验证）。
- **入库质量**（v6.41 诊断）：approved 921 / reviewing 86。详解缺失 548/921
  （59%，教师版试卷答案区只有答案无详解，源文档固有属性，已决策不生成）。
  reviewing 86 题中 82 题实际有答案（旧代码标注 + 旧文档 OCR 所致），
  30 份 PDF 重跑预计自动修复大部分。
...
1. **30 份 PDF + 9 份 DOCX 全量重跑**（用户待确认启动）：新代码（表格拆行/
   膨胀放宽/答案区豁免）逐份已验证，全量重跑刷新 30 份样本基线 + 观察
   DOCX 管线实际执行 + 自动修复 reviewing 积压（86→目标 0）。半小时轮询。
2. **PDF 基线剩余缺口（10）**：语文 3、生物 3、数学 2、物理 1、历史 1，
   重跑后复查；纯扫描件（昌平生物）另行处理，不为异常样本反复调后端。
