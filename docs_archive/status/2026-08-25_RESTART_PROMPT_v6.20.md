...
Version: 6.20
Status: 重启恢复指引（9 科答案基线 mismatch=0、严格 204/231 (88%) 基线进行中（语文 22/24、数学 22/23）；pytest 648 passed（专用测试库 aitutors_test）；修复：数学 LaTeX 归一化、语文位置行号区间校验、膨胀检测材料题识别；进行中：历史重跑（东城，试修 Q37）；T0-2 key 轮换用户决定暂缓（paddle 401 已由 mimo 回退兜底）
Date: 2026-08-25
