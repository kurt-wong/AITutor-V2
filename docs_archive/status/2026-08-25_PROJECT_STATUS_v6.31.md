...
Version: 6.32
Status: 30 份样本验证（原始质量 61%）；P2 路径260 + P4 retry500 修复；审计根因定位
Date: 2026-08-25
...
- **30 份样本验证（2026-08-25）**：原始质量 **134/219 (61%)**（零人工
  ...
  **P2**（processor._download_pdf 短文件名，解决 Windows 260 路径限制，
  ...
  retry API 500）。审计：化学 17 mismatch=同号题锚定冲突；入库缺口=
  选择题 stem_empty/锚点/answer_empty；地理育英 23 U=答案区无题号锚点。
  待修：worker LLM 挂死（P7/P10 两次，需超时/心跳）。
...
2. worker LLM 挂死（P7/P10）——LLM 调用加超时/心跳（待修）。
3. 地理报告管线幻选题号残留（23/24/25）——修报告生成侧过滤或诚实登记。
4. T0-2 泄漏 API key 轮换（用户决定暂缓）。
5. 补齐 9 科 Structure Signature，条件满足后启动 Phase 2D。
