...
- `DISPLAY_CONTRACT.md` 明确 golden 答案比较忽略全半角、引号样式、题号前缀、常见分隔符和 OCR 转义噪音。
- `run_phase1_eval.normalize_answer_text` 增强：NFKC 全半角归一、引号统一、题号前缀剥离、分隔符统一、结尾标点剥离。
- 新增归一化回归测试。
- 东城英语真实入库结果与契约 golden 对比：45/45 答案在归一化后完全匹配。
