...
def generate_report(
    ...
    for subject, acc in golden_accuracy.items():
        ...
        correct = sum(stats.get("correct", 0) for stats in acc.values())
        total = sum(stats.get("total", 0) for stats in acc.values())
        if total and correct / total < MIN_GOLDEN_ACCURACY:
            report["failures"].append(
                f"golden:{subject} overall accuracy={correct / total:.1%} < {MIN_GOLDEN_ACCURACY:.0%}"
            )
