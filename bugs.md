...
- **BUG-025 物理答案证据缺口（Resolved，验证脚本）**：物理八十中 no_answer_evidence
  3 题（Q3/Q9/Q10）+ composite_subquestion 2 题（Q15/Q16）经调查均为**验证器漏检**，
  DB 答案与证据源均正确：
  1. **空单元格答案表整行丢弃**：单选题表 10 个题号只有 8 个答案（Q4/Q7 单元格空白，
     其答案在文末"自主命制试题答案 4.A/7.B"单独给出），`_parse_plain_table` 因
     `len(nums)!=len(ans)` 丢弃整行 → Q3/Q9/Q10 无证据。修复：长度不等时按原始行结构
     重排（竖排每格一行、空行=空单元格）位置对齐；`build_evidence` 同步收集 blank_qns。
  2. **综合题子题内联答案漏检**：答案区以 `15.（1）1.50（2分）（2）不能…` 内联给出，
  ...
  3. **自共享材料误报**：Q15/Q16 管线 `shared_material == stem`（shared_line_ids ==
     stem_line_ids，实验题自带装置描述），section 归并拿首题材料误套后续题 → Q16 材料
     覆盖 7% 假阳性。修复：`verify_material` 检测自共享直接视为满足（材料即题干，
     题干已过 stem 校验）。
- **验证**：`test_answer_verifier.py` 14 passed（+3：空单元格表格恢复、综合题子题内联
  命中、部分命中仍 composite_subquestion）；物理 e2e **严格 11/20 → 16/20**、答U 9 → 4、
  材料 19/20 → 20/20；9 科基线 **209/231 → 214/231 (93%)**，其余 8 科零回归；全量
  pytest 648 passed。
- **遗留（物理）**：Q4/Q7 缺库（mimo OCR 题干为空）、Q17/Q20 free_text_answer
  （DB 答案精简版 vs 答案区完整解答 + PDF 公式竖排损坏，证据表示不可自动对齐，需人工
  核对或 OCR 重识别）。

### 2026-08-25 14:30:00

- **语文 Q17 串题人工审核（Resolved，用户方案 A：DB 数据修复）**：审核结论——
  ① 串题**真实**：L2 标注给 Q17 的 stem_line_ids（P5L014-P6L006 共 19 行）跨入下一
  section，含"四、本大题"标题（P5L023）与《到泗洪去》材料（P5L025-P6L006，即 Q18-21
  的 shared 行）；slicer 忠实切片 → stem 1840 字符；膨胀门拦截（保护生效）。
  ② 答案**无真实冲突**：DB 答案与 PDF 答案区完全一致（8 空默写），两次重灌仅内部空格
  差异 → answer_conflict 为假冲突。修复：`fix_yuwen_q17_stem.py` 将 stem 截断为仅
  默写 prompt（1840 → 228 字符）、status → approved、清 review_reason。语文
  **22/24 → 23/24**，位置 23/24 → 24/24；9 科基线 214/231 → **215/231**。
- **BUG-026 答案比较假冲突（Resolved）**：`ingestion.py` 去重答案比较 `.strip()` 只去
  首尾空白，`"①.何时可掇"` vs `"①. 何时可掇"` 内部空格差异产生假 answer_conflict。
  修复：新增 `_compact_answer()`（去全部空白含全角空格/换行）后再比对。测试 +1
  （空白差异不冲突、内容差异仍冲突）。

### 2026-08-25 16:00:00

- **遗留按序修复（6 项全部完成，基线 215/231 → 223/231）**：
  1. **物理 Q4/Q7 缺库（Resolved）**：正文第2页仅"（集团校自创题）"占位，真实题干+
     选项在第9页"自主命制试题"部分；LLM 提取了选项但题干为空被丢弃。回填
     `backfill_physics_q4_q7.py`（pdf_raw 提取题干 + 管线选项 + 答案区 4.A/7.B），
     并修正 L2 stem_start_marker（mimo 幻觉标题"滑沙项目受力与运动分析"）。验证器
     增加锚点缺失题 native 源回退（OCR 源含幻觉标题致文本跨度失败）。物理 16/20 → 20/20。
  2. **物理 Q17/Q20 free_text（Resolved）**：DB 精简版答案（"（1）a=0.2m/s²；…"）vs
     答案区完整解答（含中间步骤+分值注记）。新增 `_find_structured_answer` 分部核对
     （按（N）拆、取 "=" 后核心值、题号+子题标记锚定窗口、去分值注记），全部命中才
     matched。物理答案命中 20/20。
  3. **语文 Q24 free_text（Resolved，数据侧）**：DB 答案残缺（"例文：\n【答案】例文："，
     只截到标签）；答案区有完整作文（两篇例文+详解 4281 字符）。回填
     `fix_yuwen_q24_answer.py`。语文 23/24 → 24/24。
  4. **历史 Q26 选项 D（Resolved，数据侧）**：mimo OCR 漏识别 D 选项（OCR 里 C 后直接
     接 Q27）；pdf_raw/native 中 D"白话文在全国逐渐普及开来"真实存在。回填
     `fix_history_q26_option_d.py`。历史 42/43 → 43/43。
  5. **膨胀边界（Resolved，人工审核）**：化学 Q23/Q24（1132/1078 合法长解答题，含
     HTML 实验表格）、地理 Q26（860 读图+统计表）→ approved；物理 Q20 实为**真膨胀**
     （stem 尾部混入"自主命制试题"整节 Q4/Q7 内容 + mimo 幻觉标题 + LLM 注释），截断
     1598 → 862 字符；语文 Q23 answer_conflict 为假冲突（两次重灌 compact 一致）→
     approved。顺带修复化学 Q11 选项 B/C/D 缺失（源中真实存在，pdf_raw+OCR 重建）。
     化学 25/26 → 26/26。
  6. **英语 Q46 essay_manual_review（Resolved，验证器）**：DB 作文答案与答案区范文逐字
     一致，但作文区无 "46." 锚点（"第二节(20分) One possible version:"）。验证器对
     长文本（≥100 字符）改为"答案前 40 字符在答案区逐字出现 → matched（essay 证据）"，
     否则 essay_manual_review。英语 10/11 → 11/11。
- **新增遗留（不在本次列表）**：地理 5 题缺库（DB 25 vs L2 30）、生物 Q1/Q2 缺库
  （BUG-004）、数学 Q15 答案证据缺失（OCR 丢失负号，free_text_answer）——9 科
  223/231 (97%) 后剩余 8 题全为缺库/证据类。
- **验证**：`test_answer_verifier.py` 19 passed（+5：结构化分部命中/部分/长子段、
  essay 全文命中/未命中）；9 科基线 **215/231 → 223/231 (97%)**；全量 pytest 649+ passed。

### 2026-08-28 23:30:00

- 锁定英语/理科入库标准对抗性审查 v3.1，新增 BUG-027 Open；未开始生产代码修复。

### 2026-08-28 23:58:00

- P0-1 对抗性审查修复：补全 L2 层原始细粒度题型保留，覆盖词库合并路径；新增 T1-T5 测试，97 passed。
- 测试库修复：aitutors_test alembic version 表 stamp 到 20260827_0001，再执行 20260828_0001 migration。


### 2026-08-29 00:05:00

- P0-3 完成：多层子问递归结构上线，含 prompt 契约、L2 解析、切片、入库、API/JSONB、前端渲染；相关测试 125 passed，frontend build 通过。


### 2026-08-29 00:10:00

- P0-3 对抗性审查修复：修复前端调试残留、fallback_qno 传递、序列化安全访问；补 T1-T4 测试，129 passed。


### 2026-08-29 00:20:00

- P0-4 完成：questions 新增 answer_structure JSONB；L2/Sliced/Pipeline/Ingestion/API 全链路支持；多答案与数值范围可自动构建。
- 测试：相关集合 132 passed；迁移 20260829_0001 已应用到实际库与测试库。
